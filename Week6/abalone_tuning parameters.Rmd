...
model_dt <- rpart(target ~ . , train_data,cp=0.001)
printcp(model_dt)
plotcp(model_dt)
#On test data
...
rf <- randomForest(target ~ ., data=train_data,ntrees=1000,mtry=3)
importance(rf)
#On train data
...
```
```{r}
rf <- randomForest(target ~ ., data=train_data,ntrees=1500,mtry=3)
importance(rf)
#On train data
preds_rf_imp <- predict(rf, train_data[, !(names(train_data) %in% c("Target"))])
...
```{r}
model_knn <- knn3(target~.,train_data,k=5)
pred_knn <- predict(model_knn, train_data[, !(names(train_data) %in% c("target"))])
mae(train_data[, "target"], pred_knn)
...
```{r}
model_svm <- svm(target ~ ., data = train_data, kernal = "linear", cost=2)
summary(model_svm)
#On train data
preds_svm <- predict(model_svm, train_data[, !(names(train_data) %in% c("target"))])
